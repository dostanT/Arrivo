...
final class MapViewModel: ObservableObject {
    ...
    private func scheduleVisibleStopsUpdate() {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + Constants.updateDebounceInterval,
                                      execute: workItem)
    }
    ...
    func loadLastSelectedCoordinate() -> CLLocationCoordinate2D? {
        guard let lat = UserDefaults.standard.value(forKey: "last_selected_lat") as? Double,
              let lon = UserDefaults.standard.value(forKey: "last_selected_lon") as? Double else {
            return nil
        }
