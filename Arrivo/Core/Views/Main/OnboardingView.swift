...
struct OnboardingView: View {
    ...
    @State private var notificationDummyRequester: NotificationService? = nil
    @State private var shudelerDummyRequester: ShudelerProtocol? = nil
    ...
    func requestAll() {
        locationDummyRequester = LocationManagerClient()
        locationDummyRequester?.requestWhenInUsesAuthorization()
        notificationDummyRequester = NotificationService()
        if #available(iOS 26.0, *) {
            shudelerDummyRequester = AlarmViewModel()
        }
    }

    func reolacateAll() {
        locationDummyRequester = nil
        notificationDummyRequester = nil
        shudelerDummyRequester = nil
    }
}
