...
final class AlarmService {
    ...
    func scheduleAlarm(id: UUID, label: String, alarmConfiguration: AlarmConfiguration, duration: TimeInterval) async throws -> UUID {
    ...
    func pause(with alarmID: UUID) async throws {
        guard let alarm = alarms[alarmID],
              ...
              var model = alarmModels[alarmID] else {
    ...
    func resume(with alarmID: UUID) async throws {
        guard let alarm = alarms[alarmID],
              ...
              let remaining = model.adjustedRemaining else {
            throw AlarmError.alarmNotFound
        }
